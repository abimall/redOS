...
read -p "укажите папку монтирования(как пример /mnt/share): " mount_path
sudo mkdir "$mount_path"

    # Введите адрес шары (эксэпэл: //192.168.1.1/share$ )
read -p "Введите адрес шары(как пример: //192.168.1.1/share$ ): " share_address


    # Введите имя пользователя(без домена)
read -p "Введите имя пользователя(без домена): " username
